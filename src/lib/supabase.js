...
// Supabase defaults to localStorage, which is PER-ORIGIN — so signing in here
// left the admin app logged out, forcing a second OTP (and tripping Supabase's
// 60s "one code per minute" limit). A cookie on the PARENT domain
...
// KEEP THIS FILE IDENTICAL TO THE ADMIN APP'S copy (same AUTH_KEY = same cookie).
const AUTH_KEY = 'pv-auth'                       // MUST match the admin app
